...
int C3DTexture::textureConvertFlags_ = (GX_TRANSFER_FLIP_VERT(0) | GX_TRANSFER_OUT_TILED(1) | GX_TRANSFER_RAW_COPY(0) | GX_TRANSFER_IN_FORMAT(GX_TRANSFER_FMT_RGBA8) | GX_TRANSFER_OUT_FORMAT(GX_TRANSFER_FMT_RGBA8) | GX_TRANSFER_SCALING(GX_TRANSFER_SCALE_NO));
    ...
    for (size_t i = 0; i < bitmapSize_; i++) {
        for (size_t j = 0; j < bitmapSize_; j++) {
            ...
            *texPtr++ = bitmap_.at(i).at(j).r;
            hasTransparency_ = hasTransparency_ || bitmap_.at(i).at(j).a != 255;
        }
    }
...
bool C3DTexture::hasTransparency() const
{
